...
def getTrivials():
    ...
    #change the logic, because IsCollisionsRun is not available
    #IsCollisionsRun = int(data['isCollisionsRun']['value'])
    if ( RunKey.lower() == 'pp_run' ): IsCollisionsRun = 1
    else:
        IsCollisionsRun = 0
...
def getsnmeans():
    data = dqm_get_json(serverurl, str(RunNumber), "/Online/ALL", "SiStrip/MechanicalView/TEC/MINUS", True)
    dataPlotTECM = data['Summary_ClusterStoNCorr_OnTrack__TEC__MINUS']['rootobj']
    data = dqm_get_json(serverurl, str(RunNumber), "/Online/ALL", "SiStrip/MechanicalView/TEC/PLUS", True)
    dataPlotTECP = data['Summary_ClusterStoNCorr_OnTrack__TEC__PLUS']['rootobj']
    data = dqm_get_json(serverurl, str(RunNumber), "/Online/ALL", "SiStrip/MechanicalView/TID/MINUS", True)
    dataPlotTIDM = data['Summary_ClusterStoNCorr_OnTrack__TID__MINUS']['rootobj']
    data = dqm_get_json(serverurl, str(RunNumber), "/Online/ALL", "SiStrip/MechanicalView/TID/PLUS", True)
    dataPlotTIDP = data['Summary_ClusterStoNCorr_OnTrack__TID__PLUS']['rootobj']
    data = dqm_get_json(serverurl, str(RunNumber), "/Online/ALL", "SiStrip/MechanicalView/TIB", True)
    dataPlotTIB = data['Summary_ClusterStoNCorr_OnTrack__TIB']['rootobj']
    data = dqm_get_json(serverurl, str(RunNumber), "/Online/ALL", "SiStrip/MechanicalView/TOB", True)
    dataPlotTOB = data['Summary_ClusterStoNCorr_OnTrack__TOB']['rootobj']
    return dataPlotTECM.GetMean() < 32 and dataPlotTECP.GetMean() < 32 and dataPlotTIDP.GetMean() < 29 and dataPlotTIDM.GetMean() < 29 and dataPlotTIB.GetMean() < 29 and dataPlotTOB.GetMean() < 37
...
#commented out becaus the corresponding ME is missing
...
if ( stripHv == 1 and RunType.lower() == 'collisions' and getsnmeans() == 0 ):
    isProblem = 1
    isProblemSMS = 1
    ofile.write("Check: APV mode during collisions (S/N check):\n")
    ofile.write(" - APVs IN PEAK MODE, NOT OK! check the O2O\n")
    ofile.write("-------------------------------------------- \n")
    ofile_SMS.write("APVs in PEAK mode during collisions! Check O2O!")

##TEMPORARY test
if ( stripHv == 1 and RunType.lower() == 'collisions' and getsnmeans() == 1 ):
    ofile.write("Check: APV mode during collisions (S/N check):\n")
    ofile.write(" - APVs IN DECO MODE, OK!\n")
    ofile.write("-------------------------------------------- \n")
